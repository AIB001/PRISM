...
class MDResultsValidator:
    ...
    def _locate_md_directory(self, system_path: Path) -> Optional[Path]:
        ...
        # Strategy 2: Look for PMF system (GMX_PMF_SYSTEM) subdirectory  
        pmf_dir = system_path / "GMX_PMF_SYSTEM"
        if self._is_gmx_directory(pmf_dir):
            logger.debug(f"Found PMF system directory: {pmf_dir}")
            return pmf_dir
        
        # Strategy 3: Search all subdirectories
        for subdir in system_path.iterdir():
            if subdir.is_dir():
                # Check for GMX_PMF_SYSTEM in subdirectories
                candidate = subdir / "GMX_PMF_SYSTEM"
