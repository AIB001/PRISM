...
class PMFSystem:
    ...
    def __init__(self, system_dir: str, output_dir: str = "pmf_output",
        ...
        -----------
        system_dir : str
            Path to the built system directory (containing GMX_PMF_SYSTEM)
    ...
    def _validate_system(self):
        """Validate system directory structure - supports both PMF rebuilt and original MD systems"""
        if not self.system_dir.exists():
            raise FileNotFoundError(f"System directory not found: {self.system_dir}")
        
        # Look for GMX system structure (PMF rebuilt or original MD)
        gmx_dir = None
        system_type = None
        ...
        if (self.system_dir / "solv_ions.gro").exists():
            ...
            # Look for PMF rebuilt system first (GMX_PMF_SYSTEM)
            for candidate in self.system_dir.iterdir():
                if candidate.is_dir():
                    if candidate.name == "GMX_PMF_SYSTEM" and (candidate / "solv_ions.gro").exists():
                        gmx_dir = candidate
                        system_type = "pmf_rebuilt"
                        break
                    elif (candidate / "GMX_PMF_SYSTEM").exists():
                        pmf_candidate = candidate / "GMX_PMF_SYSTEM"
                        if (pmf_candidate / "solv_ions.gro").exists():
                            gmx_dir = pmf_candidate
                            system_type = "pmf_rebuilt"
                            break
            
            # If no PMF system found, look for original MD results
            if not gmx_dir:
                # Only look for PMF systems in subdirectories
                for candidate in self.system_dir.iterdir():
                    if candidate.is_dir() and candidate.name == "GMX_PMF_SYSTEM":
                        gmx_dir = candidate
                        system_type = "pmf_rebuilt"
                        break
        ...
        if not gmx_dir:
            raise FileNotFoundError(
                f"No PMF system (GMX_PMF_SYSTEM) found in {self.system_dir}. "
                "Please run PMF system remodeling first using pmf_remodel_only.py"
            )
        
        self.gmx_dir = gmx_dir
        self.system_type = system_type
        logger.debug(f"Found GMX directory: {self.gmx_dir} (type: {system_type})")
