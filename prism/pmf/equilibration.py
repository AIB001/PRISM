...
class PMFEquilibrationManager:
    ...
    def _validate_input_files(self):
        ...
        if not self.input_structure.exists():
            raise FileNotFoundError(f"Structure file not found: {self.input_structure}")
        
        if not self.input_topology.exists():
            raise FileNotFoundError(f"Topology file not found: {self.input_topology}")
        
        logger.debug("Input files validated successfully")
    
    def run_complete_equilibration(self) -> Dict:
        """
        Run complete equilibration workflow (EM → NVT → NPT)
    ...
    def run_nvt_equilibration(self) -> Dict:
        ...
        if not self.state['em_completed']:
            raise RuntimeError("Energy minimization must be completed before NVT")
    ...
    def run_npt_equilibration(self) -> Dict:
        ...
        if not self.state['nvt_completed']:
            raise RuntimeError("NVT equilibration must be completed before NPT")
...
"""
    ...
    def _generate_npt_mdp(self) -> Path:
        ...
        defaults = {
            'nsteps': 100000,  # 200 ps with dt=0.002
            'dt': 0.002,
            'temperature': 310.0,
            'pressure': 1.0,
            'tau_t': 0.1,
        ...
        mdp_content = f"""; npt.mdp - NPT equilibration for PMF system
title           = PMF System NPT Equilibration  
define          = -DPOSRES -DPOSRES_FC_BB=400.0 -DPOSRES_FC_SC=200.0
...
"""
    ...
    def _run_mdrun(self, tpr_file: Path, output_structure: Path, work_dir: Path,
        ...
        if result.returncode != 0:
            raise RuntimeError(f"{description} mdrun failed: {result.stderr}")
    ...
    def generate_equilibration_script(self) -> Path:
        ...
        npt_nsteps = npt_config.get('nsteps', 100000)
...
# Create NPT MDP file
cat > npt.mdp << 'EOF'
; npt.mdp - NPT equilibration for PMF system
title           = PMF System NPT Equilibration
define          = -DPOSRES -DPOSRES_FC_BB=400.0 -DPOSRES_FC_SC=200.0
