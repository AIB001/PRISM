...
class PMFBuilder:
    ...
    def build(self, frame: Optional[int] = None, equilibrate: bool = True) -> Dict:
        ...
        # Step 4: Setup equilibration based on mode
        equilibration_mode = self.config.get('equilibration', {}).get('mode', 'auto')
        
        if equilibrate:
            logger.info("=== Setting up System Equilibration ===")
            
            if equilibration_mode == 'auto':
            ...
            elif equilibration_mode == 'manual':
                logger.info("Generating manual equilibration script")
                script_results = self.generate_equilibration_script()
                results['equilibration'] = script_results
                ...
                results['final_system'] = {
                    ...
                    'equilibrated': False,
                    'script_path': script_results['script_path']
                }
