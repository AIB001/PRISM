...
Example usage:
  ...
  prism --list-forcefields
    ...
    parser.add_argument("ligands", nargs='*', help="Path(s) to ligand file(s) (MOL2/SDF). For multiple ligands, put all files before options or use --ligand-file flag")
    ...
    parser.add_argument("--ligand-file", "-lf", action='append', help="Path to ligand file (can be specified multiple times)")
    ...
    ff_group.add_argument("--water", "-w", type=str, default=None,
                          help="Water model name (default: auto-detect from force field)")
    ...
    ff_group.add_argument("--ligand-number", "-n", type=int, default=None,
                          help="Number of ligands (default: auto-detect from input files)")
    ff_group.add_argument("--ligand-charge", type=int, default=0,
                          help="Net charge of ligand (default: 0, applies to all ligands)")
    ff_group.add_argument("--forcefield-path", "-ffp", type=str, action='append', default=None,
    ...
    ff_group.add_argument("--nter", type=str, default=None,
    ...
    ff_group.add_argument("--cter", type=str, default=None,
    ...
    gaussian_group.add_argument("--gaussian", "-g",
        choices=['hf', 'dft'], default=None,
        help="Enable Gaussian RESP charge calculation: 'hf' (HF/6-31G*) or 'dft' (B3LYP/6-31G*). Default: disabled (uses AM1-BCC)")
    gaussian_group.add_argument("--isopt",
        choices=['true', 'false'], default='false',
        help="Perform geometry optimization before ESP calculation (default: false)")
    gaussian_group.add_argument("--respfile", "-rf",
        action='append', default=None,
        help="Path to existing RESP mol2 file. For multiple ligands, specify once per ligand: -rf resp1.mol2 -rf resp2.mol2")
    gaussian_group.add_argument("--nproc", type=int, default=16,
        help="Number of processors for Gaussian calculation (default: 16)")
    gaussian_group.add_argument("--mem", type=str, default='4GB',
        help="Memory allocation for Gaussian calculation (default: 4GB)")
    ...
    sim_group.add_argument("--protonation", "-proton", choices=['gromacs', 'propka'], default='gromacs',
    ...
    mmpbsa_group.add_argument("--mmpbsa-traj", type=float, default=None, metavar="NS",
    ...
    mmpbsa_group.add_argument("--gmx2amber", action="store_true",
    ...
    pmf_group.add_argument("--pmf", action="store_true",
    ...
    pmf_group.add_argument("--pull-vector", "-pullvec", type=int, nargs=2, metavar=('PROT_ATOM', 'LIG_ATOM'),
    ...
    pmf_group.add_argument("--box-extension", "-boxext", type=float, nargs=3, metavar=('X', 'Y', 'Z'),
                           default=[0.0, 0.0, 2.0],
                           help="Box extension in X, Y, Z directions (nm) for PMF mode. "
                                "Default: 0.0 0.0 2.0 (extends Z by 2 nm for pulling space)")
    pmf_group.add_argument("--umbrella-time", type=float, default=10.0,
                           help="Simulation time per umbrella window in ns (default: 10.0)")
    pmf_group.add_argument("--umbrella-spacing", type=float, default=0.12,
                           help="Spacing between umbrella windows in nm (default: 0.12)")
    pmf_group.add_argument("--wham-begin", type=int, default=1000,
                           help="Time in ps to discard for WHAM equilibration (default: 1000)")
    pmf_group.add_argument("--wham-bootstrap", type=int, default=200,
    ...
    rest2_group.add_argument("--rest2", action="store_true",
                             help="Enable REST2 mode: build MD system then generate REST2 replica exchange setup")
    rest2_group.add_argument("--t-ref", type=float, default=310.0,
                             help="Reference temperature in K (default: 310)")
    rest2_group.add_argument("--t-max", type=float, default=450.0,
                             help="Maximum effective temperature in K (default: 450)")
    rest2_group.add_argument("--replica-number", type=int, default=16,
                             help="Number of REST2 replicas (default: 16)")
    rest2_group.add_argument("--rest2-cutoff", type=float, default=0.5,
                             help="Pocket detection cutoff in nm for REST2 (default: 0.5)")
...
"""
    ...
    if not ligand_paths:
        parser.error("At least one ligand file is required. Provide ligand path(s) as positional arguments or use --ligand-file flag")
    ...
    if args.ligand_number is not None:
        if args.ligand_number != actual_ligand_count:
            parser.error(f"--ligand-number (-n) specified {args.ligand_number} but {actual_ligand_count} ligand file(s) provided")
