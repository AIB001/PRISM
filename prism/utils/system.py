...
class SystemBuilder:
    ...
    def _combine_protein_ligand(self, protein_gro: str, lig_ff_dir: str) -> str:
        ...
        with open(protein_gro, "r") as f_prot, open(lig_gro_path, "r") as f_lig, open(complex_gro, "w") as f_out:
            ...
            f_out.writelines(prot_lines[2:-1])  # Protein coordinates
            f_out.writelines(lig_lines[2:-1])  # Ligand coordinates
    ...
    def _update_topology_molecules(self, topol_path: str, genion_stdout: str):
        ...
        """
        print("Updating topology with new molecule counts from genion...")

        mol_counts = {}
        # Regex to find lines like 'SOL              :  11523' or 'CL               :      8'
        pattern = re.compile(r'^\s*([A-Z0-9_]+)\s+:\s+(\d+)')

        # Find the summary table in genion's output
        parsing = False
        for line in genion_stdout.splitlines():
            ...
            if parsing:
                ...
                elif not line.strip():  # Stop at the first blank line after the table
                    break

        if not mol_counts:
            print("Warning: Could not parse molecule counts from genion output. Topology might be incorrect.")
            return
