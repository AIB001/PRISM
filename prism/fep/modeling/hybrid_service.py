...
class HybridBuildService:
    ...
    @staticmethod
    def write_hybrid_gro(output_gro: str | Path, hybrid_atoms: list, ref_atoms: list, mut_atoms: list) -> Path:
        """Write a hybrid GRO using explicit source-index mapping when available."""
        ...
        for hatom in hybrid_atoms:
            ...
            if coord is None:
                if hatom.name.endswith("A"):
                    coord = ref_coord_map.get(hatom.name[:-1])
                elif hatom.name.endswith("B"):
                    coord = mut_coord_map.get(hatom.name[:-1])
                else:
        ...
        with open(output_path, "w") as handle:
            handle.write("Hybrid ligand structure generated by PRISM-FEP\n")
            handle.write(f"{len(hybrid_atoms_data)}\n")
            for atom_data in hybrid_atoms_data:
                idx = atom_data["index"]
                name = atom_data["name"]
                x, y, z = atom_data["coord"]
                handle.write(f"{1:5d}LIG  {name:>5s}{idx:5d}{x:8.3f}{y:8.3f}{z:8.3f}\n")
    ...
    def build_from_forcefield_dirs(
        ...
        hybrid_gro = self.write_hybrid_gro(hybrid_dir / "hybrid.gro", hybrid_atoms, ref_atoms, mut_atoms)
