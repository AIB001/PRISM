...
class NAMDFEPAnalyzer:
    ...
    def __init__(self, config: AnalysisConfig):
        ...
        if not Path(self._script_path).exists():
            raise FileNotFoundError(
                f"NAMD FEP decomposition script not found: {self._script_path}"
            )
    ...
    def parse_fepout_summary(self, fepout_file: str) -> Dict[str, float]:
        ...
        try:
            ...
            summary = {
                'lambda2': float(last_line[8]),
                'dG': float(last_line[11]),
                'net_change': float(last_line[18])
            }
    ...
    def calculate_system_decomposition(self,
                                      system_dir: str,
                                      system_type: str = 'auto',
                                      n_repeats: int = 4,
                                      parallel: bool = True,
                                      cache_name: Optional[str] = None,
                                      **decomp_kwargs) -> Dict[str, np.ndarray]:
        """
        ...
        **decomp_kwargs
        ...
        for sys_type in system_types:
            ...
            repeat_dirs = sorted([
                d for d in system_dir.iterdir()
                if d.is_dir() and d.name.startswith(sys_type)
            ])
    ...
    def calculate_convergence_data(self,
                                  fepout_files: Union[str, List[str]],
                                  num_points: int = 10,
                                  cache_name: Optional[str] = None,
                                  **decomp_kwargs) -> pd.DataFrame:
        """
    ...
    def batch_calculate_systems(self,
                               base_dir: str,
                               system_pattern: str,
        ...
        **kwargs
        ...
        for system_dir in system_dirs:
            ...
            try:
                system_results = self.calculate_system_decomposition(
                    str(system_dir),
                    cache_name=system_name,
                    **kwargs
                )
    ...
    def batch_process_fep_system(self,
                                 system_dir: str,
                                 output_dir: Optional[str] = None,
                                 save_data: bool = True,
                                 **kwargs) -> Dict:
        ...
        **kwargs
        ...
        for phase in ['complex', 'ligand']:
            ...
            for repeat_dir in repeat_dirs:
                ...
                try:
                    # Parse raw data
                    raw_data = self.parse_fepout_raw(
                        str(fepout_file),
                        cache_name=f"{phase}_{repeat_dir.name}"
                    )
