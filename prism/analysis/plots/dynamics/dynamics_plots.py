    ...
    try:
        ...
        for i, (data, name) in enumerate(zip(dynamics_data_list, trajectory_names)):
            if data is not None and 'ligand_rmsd' in data and 'time' in data:
                ...
                ax1.plot(time_ns, data['ligand_rmsd'],
                        color=colors[i % 3], label=name, linewidth=1.5, alpha=0.8)
        ...
        if key_res:
            for i, (data, name) in enumerate(zip(dynamics_data_list, trajectory_names)):
                if data and 'distances' in data and key_res in data['distances']:
                    ...
                    ax2.plot(time_ns, distances_ang,
                            color=colors[i % 3], label=name, linewidth=1.5, alpha=0.8)
            ...
            ax2.text(0.5, 0.5, 'No Distance Data', ha='center', va='center',
                    transform=ax2.transAxes, fontsize=16, fontfamily='Times New Roman')
        ...
        if has_pocket_data:
            ...
            ax3.text(0.5, 0.5, 'No Pocket Volume Data', ha='center', va='center',
                    transform=ax3.transAxes, fontsize=16, fontfamily='Times New Roman')
        ...
        if all_key_residues:
            ...
            for i, res in enumerate(top_residues):
                ...
                if distances_all:
                    ax4.hist(distances_all, bins=20, alpha=0.6,
                            label=f'Lig-{res}', density=True, color=colors_res[i % 3])
            ...
            ax4.text(0.5, 0.5, 'No Distance Distribution', ha='center', va='center',
...
def plot_ligand_rmsd_comparison(dynamics_data_list: List[Dict],
                                trajectory_names: List[str],
                                output_path: str,
                                title: str = "") -> bool:
    """
    ...
    try:
        ...
        for i, (data, name) in enumerate(zip(dynamics_data_list, trajectory_names)):
            if data and 'ligand_rmsd' in data and 'time' in data:
                time_ns = data['time'] / 1000.0
                ax.plot(time_ns, data['ligand_rmsd'],
                       color=colors[i % 3], label=name, linewidth=2, alpha=0.8)
...
def plot_key_distances_timeseries(dynamics_data_list: List[Dict],
                                  trajectory_names: List[str],
                                  output_path: str,
