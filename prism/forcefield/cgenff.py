...
class CGenFFForceFieldGenerator(ForceFieldGeneratorBase):
    ...
    def _find_input_files(self):
        ...
        if not pdb_files:
            raise FileNotFoundError(f"No PDB file found in {self.cgenff_dir}")
        if not top_files:
            raise FileNotFoundError(f"No TOP file found in {self.cgenff_dir}")
    ...
    def _generate_lig_itp(self):
        """Generate LIG.itp"""
        # Include CGenFF bonded parameters if they exist
        lig_ffbonded = self.cgenff_dir / "charmm36.ff" / "lig_ffbonded.itp"
        content = ""
        if lig_ffbonded.exists():
            content += "; Include CGenFF ligand-specific bonded parameters\n"
            content += '#include "charmm36.ff/lig_ffbonded.itp"\n\n'
    ...
    def _generate_lig_top(self):
        ...
        if charmm_ff_dir.exists():
            ...
            lig_ffbonded = charmm_ff_dir / "lig_ffbonded.itp"
            if lig_ffbonded.exists():
                content += '#include "charmm36.ff/lig_ffbonded.itp"\n'
            ...
            content += "     1\t     2\tyes   \t0.500000\t0.833333\n\n\n"
