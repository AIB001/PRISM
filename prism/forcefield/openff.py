...
try:
    ...
    class OpenFFForceFieldGenerator:
        def __init__(self, *args, **kwargs):
            _ = (args, kwargs)
            raise ImportError(
                "OpenFF dependencies not installed.\n"
...
class OpenFFForceFieldGenerator(ForceFieldGeneratorBase):
    ...
    def run(self):
        ...
        try:
            # Check if output already exists BEFORE trying to load molecule
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            try:
                ...
                cmd = [
                    "antechamber",
                    "-i",
                    "input.mol2",  # Use relative path in temp dir
                    "-fi",
                    "mol2",
                    "-o",
                    "output.sdf",  # Use relative path in temp dir
                    "-fo",
                    "sdf",
                    "-c",
                    "bcc",  # Use BCC charge method instead of 'wc'
                    "-nc",
                    str(self.charge),
                ]
            ...
            # If all methods fail
            raise RuntimeError(
                "Could not convert MOL2 file. Please check the input file and ensure antechamber or obabel is installed."
            )
