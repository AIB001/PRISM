...
def validate_gmx_directory(gmx_dir):
    ...
    if missing:
        raise FileNotFoundError(f"Missing required files in {gmx_dir}: {', '.join(missing)}")
...
def find_forcefield_dir(output_dir):
    ...
    available_dirs = [d for d in os.listdir(output_dir) if os.path.isdir(os.path.join(output_dir, d))]
...
def check_gpu_available():
    ...
    """
    gpu_info = {"cuda_available": False, "opencl_available": False, "devices": []}
...
def estimate_simulation_time(mdp_params, system_size):
    ...
    cpu_performance = {"em": 100000, "nvt": 5000, "npt": 4500, "prod": 4000}
